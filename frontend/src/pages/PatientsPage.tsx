...
export function PatientsPage() {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors, isSubmitting } } = useForm<PatientForm>({
  ...
  function openCreate() {
    setEditing(null);
    reset({ nombres: '', apellidos: '', tipoDocumento: 'DNI', documentoIdentidad: '', sexo: undefined as unknown as Sexo, telefono: '', email: '', direccion: '' });
    setShowForm(true);
  }
          ...
          <tbody>
            ...
            {patients.map((p) => (
              <tr key={p.id}>
                ...
                <td style={ui.td}>{TIPO_DOCUMENTO_LABEL[p.tipoDocumento]}: {p.documentoIdentidad}</td>
      ...
      {showForm && (
        <Modal title={editing ? 'Editar paciente' : 'Nuevo paciente'} onClose={closeForm}>
          <form onSubmit={handleSubmit((values) => saveMutation.mutate(values))}>
            <label>Nombres</label>
            <input {...register('nombres', { onBlur: alSalirCapitalizar('nombres') })} placeholder="Ej. María José" style={ui.input} />
            {errors.nombres && <small style={{ color: 'var(--color-critical)' }}>{errors.nombres.message}</small>}

            <label>Apellidos</label>
            <input {...register('apellidos', { onBlur: alSalirCapitalizar('apellidos') })} placeholder="Ej. García López" style={ui.input} />
            ...
            <input
              ...
              placeholder={watchedTipoDocumento === 'DNI' ? 'Ej. 12345678' : 'Ej. AB123456'}
              style={ui.input}
            />
